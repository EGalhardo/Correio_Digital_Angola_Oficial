...
function JitsiEmbed({ roomName, subject, isActive }: JitsiEmbedProps) {
  ...
  if (!isActive) {
    return (
      <div className="bg-slate-950 border border-slate-700 rounded-2xl overflow-hidden relative shadow-lg">
  ...
  return (
    <div className="bg-slate-950 border border-slate-700 rounded-2xl overflow-hidden relative shadow-xl">
...
// mockSessions com 3 atendimentos disponíveis
const mockSessions = [
  {
    id: 'sessao-001',
...
export function VideoSessionPage({ onBack, addAuditLog }: VideoSessionPageProps) {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      // Se não houver sessões, usar mockSessions
      setSessions(allSessions.length > 0 ? allSessions : mockSessions);
    } catch (e) {
      // Em caso de erro, usar mockSessions
                  ...
                  .map(session => {
                    ...
                    return (
                      <motion.div
                        ...
                        onClick={() => session.status !== 'em_curso' && handleStartCall(session)}
                      >
        ...
        <div className="space-y-4">
          {selectedSession ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-3">
              ...
              {/* Jitsi Embed visível na coluna direita */}
              <JitsiEmbed roomName={selectedSession.roomName || `cda-atendimento-${selectedSession.id}`} subject={selectedSession.subject} isActive={isInCall} />
