...
export class OfflineManager {
  ...
  public static createAutomaticBackup(): BackupData {
    ...
    // Collect all government system states for backup
    ...
    // Keep only last 5 backups for space efficiency
    index.unshift(newBackup);
    if (index.length > 5) {
      const discarded = index.pop();
      if (discarded) {
        const discKey = `gov_backup_${new Date(discarded.timestamp).getTime()}`;
